...
__global__ void postprocess_kernel(const float* cls_input, float* box_input, const float* dir_input, float* anchors, float* anchor_bottom_heights, float* bndbox_output, float* score_output, int* object_counter, const float min_x_range, const float max_x_range, const float min_y_range, const float max_y_range, const int feature_x_size, const int feature_y_size, const int num_anchors, const int num_classes, const int num_box_values, const float score_thresh, const float dir_offset) {
...
cudaError_t postprocess_launch(const float* cls_input, float* box_input, const float* dir_input, float* anchors, float* anchor_bottom_heights, float* bndbox_output, float* score_output, int* object_counter, const float min_x_range, const float max_x_range, const float min_y_range, const float max_y_range, const int feature_x_size, const int feature_y_size, const int num_anchors, const int num_classes, const int num_box_values, const float score_thresh, const float dir_offset, cudaStream_t stream) {
    int bev_size = feature_x_size * feature_y_size;
    dim3 threads(num_anchors);
    dim3 blocks(bev_size);

    postprocess_kernel<<<blocks, threads, 0, stream>>>(cls_input, box_input, dir_input, anchors, anchor_bottom_heights, bndbox_output, score_output, object_counter, min_x_range, max_x_range, min_y_range, max_y_range, feature_x_size, feature_y_size, num_anchors, num_classes, num_box_values, score_thresh, dir_offset);
...
__device__ inline bool devIoU(float const* const box_a, float const* const box_b, const float nms_thresh) {
    float a_angle = box_a[6];
    float b_angle = box_b[6];
    float a_dx_half = box_a[3] / 2;
    float b_dx_half = box_b[3] / 2;
    float a_dy_half = box_a[4] / 2;
    float b_dy_half = box_b[4] / 2;
    float a_x1 = box_a[0] - a_dx_half;
    float a_y1 = box_a[1] - a_dy_half;
    float a_x2 = box_a[0] + a_dx_half;
    float a_y2 = box_a[1] + a_dy_half;
    float b_x1 = box_b[0] - b_dx_half;
    float b_y1 = box_b[1] - b_dy_half;
    float b_x2 = box_b[0] + b_dx_half;
    float b_y2 = box_b[1] + b_dy_half;
    ...
    float a_angle_cos = cos(a_angle);
    float a_angle_sin = sin(a_angle);
    float b_angle_cos = cos(b_angle);
    float b_angle_sin = sin(b_angle);
    ...
    for(int j = 0; j < cnt - 1; j++) {
        for(int i = 0; i < cnt - j - 1; i++) {
            if(atan2(cross_points[i].y - poly_center.y, cross_points[i].x - poly_center.x) > atan2(cross_points[i + 1].y - poly_center.y, cross_points[i + 1].x - poly_center.x)) {
                temp = cross_points[i];
...
cudaError_t nms_launch(unsigned int boxes_num, float* boxes, float nms_thresh, uint64_t* mask, cudaStream_t stream) {
