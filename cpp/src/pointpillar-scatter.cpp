...
nvinfer1::DimsExprs PPScatterPlugin::getOutputDimensions(int outputIndex,
...
bool PPScatterPlugin::supportsFormatCombination(int pos, const nvinfer1::PluginTensorDesc* inOut, int nbInputs, int nbOutputs) noexcept {
    assert(nbInputs == 3);
    assert(nbOutputs == 1);
...
void PPScatterPlugin::configurePlugin(const nvinfer1::DynamicPluginTensorDesc* in,
                                    int nbInputs,
...
size_t PPScatterPlugin::getWorkspaceSize(const nvinfer1::PluginTensorDesc* inputs,
                                        int nbInputs,
...
int PPScatterPlugin::enqueue(const nvinfer1::PluginTensorDesc* inputDesc,
    ...
    try {
        ...
        if(inputType == nvinfer1::DataType::kHALF) {
            ...
            status = pillarScatterHalfKernelLaunch(pillar_features_data, coords_data, params_data, featureX, featureY, spatial_feature_data, stream);
            assert(status == 0);
            return status;
            ...
            status = pillarScatterFloatKernelLaunch(pillar_features_data, coords_data, params_data, featureX, featureY, spatial_feature_data, stream);
            assert(status == 0);
            return status;
        } else {
            assert(status == 0);
...
nvinfer1::DataType PPScatterPlugin::getOutputDataType(int index, const nvinfer1::DataType* inputTypes, int nbInputs) const noexcept {
    return inputTypes[0];
}
...
IPluginV2* PPScatterPluginCreator::deserializePlugin(const char* name, const void* serialData, size_t serialLength) noexcept {
