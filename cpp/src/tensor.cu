...
Tensor Tensor::loadbinary(const std::string& file, std::vector<int64_t> shape, DataType dtype, bool device) {
    ...
    if(num_implicit_dim > 0) {
        ...
        if(fsize % explicit_bytes != 0) {
            printf("Cannot be calculated the implicit dimension, and the file bytes[%ld] cannot be divided by the size of the explicit dimension bytes[%ld]\n", fsize, explicit_bytes);
            fclose(f);
            return Tensor();
        }
    ...
    if(bytes != fsize) {
        printf("Cannot be loaded by the specified shape, The file has %ld byte and the shape requires %ld byte\n", fsize, bytes);
        fclose(f);
        return Tensor();
    }
...
Tensor Tensor::load(const std::string& file, bool device) {
    ...
    std::transform(dims, dims + ndim, shape.begin(), [](int x) {
        return x;
    });
...
bool Tensor::save(const std::string& file, void* stream_) const {
    ...
    int dims[16];
    std::transform(this->shape.begin(), this->shape.end(), dims, [](int64_t i) -> int {
        return i;
    });
...
void Tensor::self_byte_check(size_t type_bytes) const {
    ...
    if(self_bytes != type_bytes) {
        this->print("This");
        Assertf(self_bytes == type_bytes, "Failed to check the data type, your code may have a logic error, The type of this tensor is %d bytes, but the pointer is %d bytes.", static_cast<int>(self_bytes), static_cast<int>(type_bytes));
    }
}
