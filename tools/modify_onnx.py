...
def replace_with_clip(self,
    ...
    return self.layer(name="PPScatter_0",
                      op="PPScatterPlugin",
                      inputs=inputs,
                      outputs=outputs,
                      attrs=op_attrs)
...
def simplify_postprocess(onnx_model):
    ...
    cls_preds = gs.Variable(name="cls_preds",
                            dtype=np.float32,
                            shape=(1, 248, 216, 18))
    box_preds = gs.Variable(name="box_preds",
                            dtype=np.float32,
                            shape=(1, 248, 216, 42))
    dir_cls_preds = gs.Variable(name="dir_cls_preds",
                                dtype=np.float32,
                                shape=(1, 248, 216, 12))
    ...
    concat_node = loop_node(graph,
                            first_ConvTranspose_node,
                            3)
    ...
    for i in range(3):
        transpose_node = loop_node(graph,
                                   first_node_after_concat[i],
                                   1)
...
def simplify_preprocess(onnx_model):
    ...
    input_new = gs.Variable(name="voxels",
                            dtype=np.float32,
                            shape=(MAX_VOXELS, 32, 10))
    
    X = gs.Variable(name="voxel_idxs",
                    dtype=np.int32,
                    shape=(MAX_VOXELS, 4))
    
    Y = gs.Variable(name="voxel_num",
                    dtype=np.int32,
                    shape=(1, ))
    
    reshape_0 = gs.Node(name="reshape_0",
                        op="Reshape")
    reshape_0.inputs.append(input_new)
    reshape_0_shape = gs.Constant(name="reshape_0_shape",
                                  values=np.array([MAX_VOXELS * 32, 10],
                                                  dtype=np.int64))
    reshape_0.inputs.append(reshape_0_shape)
    reshape_0_out = gs.Variable(name="reshape_0_out",
                                shape=[MAX_VOXELS * 32, 10],
                                dtype=np.float32)
    ...
    matmul_op_out = gs.Variable(name="matmul_op_out",
                                shape=[MAX_VOXELS * 32, 64],
                                dtype=np.float32)
    ...
    bn_op_out = gs.Variable(name="bn_op_out",
                            shape=[MAX_VOXELS * 32, 64],
                            dtype=np.float32)
    ...
    relu_op_out = gs.Variable(name="relu_op_out",
                              shape=[MAX_VOXELS * 32, 64],
                              dtype=np.float32)
    relu_op.outputs[0] = relu_op_out

    reshape_1 = gs.Node(name="reshape_1",
                        op="Reshape")
    reshape_1.inputs.append(relu_op_out)
    reshape_1_shape = gs.Constant(name="reshape_1_shape",
                                  values=np.array([MAX_VOXELS, 32, 64],
                                                  dtype=np.int64))
    reshape_1.inputs.append(reshape_1_shape)
    reshape_1_out = gs.Variable(name="reshape_1_out",
                                shape=[MAX_VOXELS, 32, 64],
                                dtype=np.float32)
    ...
    graph.replace_with_clip([reducemax_op.outputs[0], X, Y],
                            [conv_op.inputs[0]])
